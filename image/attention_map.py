...
def extract(image: np.ndarray, image_id: str) -> Dict[str, Any]:
    ...
    try:
        ...
        sample_duration = float(os.getenv("TPPGAZE_DURATION_SEC", "3.0"))
        ...
        # Trust TPP-Gaze: pixel coords, durations in ms
        scanpaths = [sp.astype(np.float32) for sp in scanpaths if sp is not None]

        # Clamp to bounds only (no rescale/swap)
        for sp in scanpaths:
            sp[:, 0] = np.clip(sp[:, 0], 0, W - 1)
            sp[:, 1] = np.clip(sp[:, 1], 0, H - 1)

        # Build heatmap/peaks once
        USE_DURATION = os.getenv("TPPGAZE_USE_DURATION", "1") != "0"
        DURATION_GAMMA = float(os.getenv("TPPGAZE_DURATION_GAMMA", "0.5"))
        SIGMA_PX = float(os.getenv("TPPGAZE_SIGMA_PX", "10"))
        NMS_RADIUS = int(os.getenv("TPPGAZE_NMS_RADIUS", "6"))
